...
const Order = () => {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch('http://localhost:5000/merchandiser/orderlist', {
        method: 'POST',
        headers: {
  ...
  const fetchProductDetails = async (productIds) => {
    try {
      const response = await fetch('http://localhost:5000/merchandiser/orderlist/products', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const fetchUserDetails = async (userId) => {
    try {
      const response = await fetch('http://localhost:5000/merchandiser/order/user/details', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleStatusUpdate = async (order_date,order_time,user_id, newStatus) => {
    try {
      console.log('Updating order status:', { order_date,order_time, user_id, newStatus });
      const response = await fetch('http://localhost:5000/merchandiser/order/update', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleCancel = async (order_date,order_time,user_id) => {
    try {
      console.log('Cancelling order status:', { order_date,order_time, user_id});
      ...
      <div className="order-requests">
        {orders.length > 0 ? (
          <ul className="order-list">
            {orders.map((order) => (
              <li key={order.ORDER_ID} className={`order-item ${order.DELIVERY_STATUS.toLowerCase()}`}>
                ...
                <div className="order-details1">
                  ...
                  <div className="order-details1-lower">
                    <p className="order-details1-lower-date">{order.ORDER_DATE} {order.ORDER_TIME}</p>
                    <p className="order-details1-lower-price">Total: ${calculateTotalPrice(order.ORDER_DETAILS).toFixed(2)} </p>
                  </div>
                </div>
                
                <div className="order-details2">
                  <div className="order-details2-upper">
                  <h2 className="order-details2-upper-title">Customer Details </h2>
                  <p><strong>Order Date:</strong> <span className="order-details2-label">{order.ORDER_DATE} {order.ORDER_TIME}</span></p>
