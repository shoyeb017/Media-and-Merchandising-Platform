...
const MerchandiserCollaborate = () => {
  ...
  const navigate = useNavigate();
   ...
   const fetchCollaborationsStatus = async () => {
    try {
      const response = await fetch('http://localhost:5000/merchandiser/collaborate/status', {
        method: 'POST',
        headers: {
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/merchandiser/collaborate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div>
        <div className="addcollaborate-container"></div>
        ...
        <div className="form-group">
            ...
            {filteredCompanies.map((company) => (
                <div
                key={company.COM_ID}
                className={`company-item ${selectedCompany && selectedCompany.COM_ID === company.COM_ID ? 'selected' : ''}`}
        ...
        <button className="submit-button" onClick={handleRequest}>Sent Request</button>
        </div>
            ...
            {collaborationsStatus.map((collab) => (
              <li key={collab.MER_ID} className={`collab-item ${collab.C_STATUS.toLowerCase()}`}>
                ...
                <div className="collab-details3">
                  <button class="collab-details3-button" onClick={() => navigate(`details/${collab.COM_ID}`)}>
